...
public sealed class SecureShiftClient : ISecureShiftClient
{
    ...
    private readonly SecureShiftOptions opt;
    ...
    public async Task<PriceResult?> GetSellPriceAsync(PriceQuery query, CancellationToken ct = default)
    {
        ...
        var estimated = await GetEstimateAsync(
            currencyFrom: opt.XmrSymbol,
            networkFrom: opt.XmrNetwork,
            currencyTo: opt.UsdtSymbol,
            networkTo: opt.UsdtNetwork,
            amount: 1m,
            ct);
    ...
    public async Task<PriceResult?> GetBuyPriceAsync(PriceQuery query, CancellationToken ct = default)
    {
        ...
        var estimated = await GetEstimateAsync(
            currencyFrom: opt.UsdtSymbol,
            networkFrom: opt.UsdtNetwork,
            currencyTo: opt.XmrSymbol,
            networkTo: opt.XmrNetwork,
            amount: probe,
            ct);
    ...
    public async Task<IReadOnlyList<ExchangeCurrency>> GetCurrenciesAsync(CancellationToken ct = default)
    {
        ...
        var qs = $"get-pairs?symbol={Uri.EscapeDataString(opt.XmrSymbol)}&network={Uri.EscapeDataString(opt.XmrNetwork)}";
