...
public sealed class BitcoinVNClient : IBitcoinVNClient
{
    ...
    private readonly BitcoinVNOptions opt;
    ...
    // ─── Currencies ─────────────────────────────────────────────────────────
    // GET /api/info returns { assets: { "BTC": {id,name,tags}, ... }, transferMethods }.
    // The assets dict is the list of supported coins; fiat (VND/USD/EUR…) is excluded.
    public async Task<IReadOnlyList<ExchangeCurrency>> GetCurrenciesAsync(CancellationToken ct = default)
    {
        ...
        InfoDto? info;
        try { info = JsonSerializer.Deserialize<InfoDto>(res.Body, JsonOpt); }
        catch { return Array.Empty<ExchangeCurrency>(); }

        if (info?.Assets is null || info.Assets.Count == 0)
            return Array.Empty<ExchangeCurrency>();

        return info.Assets
            .Where(kv => kv.Value is not null
                         && !(kv.Value!.Tags?.Contains("fiat", StringComparer.OrdinalIgnoreCase) ?? false))
            .Select(kv =>
            {
                var code = (kv.Value!.Id ?? kv.Key).Trim().ToUpperInvariant();
    ...
    {
        [JsonPropertyName("assets")] public Dictionary<string, AssetDto>? Assets { get; set; }
    }
