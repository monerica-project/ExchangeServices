...
public sealed class CypherGoatOptions
{
    ...
    // The CypherGoat API (api.cyphergoat.com) exposes no coins/currencies endpoint —
    // only /estimate, /swap, /transaction. The full supported-coin list is rendered
    // server-side into the public homepage as <div data-ticker data-network> elements
    // inside the coin-selection modal, so the catalog is scraped from there.
    public string CoinListUrl { get; set; } = "https://cyphergoat.com/";
