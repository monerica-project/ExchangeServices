...
public static class ServiceCollectionExtensions
{
    ...
    public static IServiceCollection AddNexchange(this IServiceCollection services, IConfiguration config)
    {
        services.Configure<NexchangeOptions>(config.GetSection("Nexchange"));

        services.AddHttpClient<INexchangeClient, NexchangeClient>((sp, client) =>
        {
            var opt = sp.GetRequiredService<IOptions<NexchangeOptions>>().Value;
            client.BaseAddress = new Uri(opt.BaseUrl.TrimEnd('/') + "/");
            client.Timeout = Timeout.InfiniteTimeSpan; // per-request timeout handled in the client
        });

        services.AddTransient<IExchangePriceApi>(sp => sp.GetRequiredService<INexchangeClient>());
        services.AddTransient<IExchangeBuyPriceApi>(sp => sp.GetRequiredService<INexchangeClient>());
        services.AddTransient<IExchangeCurrencyApi>(sp => sp.GetRequiredService<INexchangeClient>());
    ...
    public static IServiceCollection AddTrocador(this IServiceCollection services, IConfiguration config)
    {
        // ── Trocador ──────────────────────────────────────────────────────────────────

        services.Configure<TrocadorOptions>(
            config.GetSection("Trocador"));

        services.AddHttpClient<ITrocadorClient, TrocadorClient>(client =>
        {
            var baseUrl = config["Trocador:BaseUrl"] ?? "https://api.trocador.app";
            client.BaseAddress = new Uri(baseUrl.TrimEnd('/') + "/");

            var seconds = config.GetValue<int>("Trocador:RequestTimeoutSeconds", 12);
            client.Timeout = TimeSpan.FromSeconds(Math.Clamp(seconds, 2, 60));
        });

        services.AddTransient<IExchangePriceApi>(sp => sp.GetRequiredService<ITrocadorClient>());
        services.AddTransient<IExchangeBuyPriceApi>(sp => sp.GetRequiredService<ITrocadorClient>());
        services.AddTransient<IExchangeCurrencyApi>(sp => sp.GetRequiredService<ITrocadorClient>());

        return services;
    }

    public static IServiceCollection AddStereoSwap(this IServiceCollection services, IConfiguration config)
    {


        // ── StereoSwap ────────────────────────────────────────────────────────────────

        services.Configure<StereoSwapOptions>(
        config.GetSection("StereoSwap"));

        services.AddHttpClient<IStereoSwapClient, StereoSwapClient>(client =>
        {
            var seconds = config.GetValue<int>("StereoSwap:RequestTimeoutSeconds", 10);
    ...
    public static IServiceCollection AddBitXChange(this IServiceCollection services, IConfiguration config)
    {

        // ── BitXChange ────────────────────────────────────────────────────────────────

        services.Configure<BitXChangeOptions>(
        config.GetSection("BitXChange"));

        services.AddHttpClient<IBitXChangeClient, BitXChangeClient>(client =>
        {
            var seconds = config.GetValue<int>("BitXChange:RequestTimeoutSeconds", 10);
            client.Timeout = TimeSpan.FromSeconds(Math.Clamp(seconds, 2, 30));
        });

        services.AddTransient<IExchangePriceApi>(sp => sp.GetRequiredService<IBitXChangeClient>());
        services.AddTransient<IExchangeBuyPriceApi>(sp => sp.GetRequiredService<IBitXChangeClient>());
        services.AddTransient<IExchangeCurrencyApi>(sp => sp.GetRequiredService<IBitXChangeClient>());
        return services;
    }

    public static IServiceCollection AddSwapter(this IServiceCollection services, IConfiguration config)
    {

        // ── Swapter ───────────────────────────────────────────────────────────────────

        services.Configure<SwapterOptions>(
       config.GetSection("Swapter"));

        services.AddHttpClient<ISwapterClient, SwapterClient>(client =>
        {
            var seconds = config.GetValue<int>("Swapter:RequestTimeoutSeconds", 10);
            client.Timeout = TimeSpan.FromSeconds(Math.Clamp(seconds, 2, 30));
        });

        services.AddTransient<IExchangePriceApi>(sp => sp.GetRequiredService<ISwapterClient>());
        services.AddTransient<IExchangeBuyPriceApi>(sp => sp.GetRequiredService<ISwapterClient>());
        services.AddTransient<IExchangeCurrencyApi>(sp => sp.GetRequiredService<ISwapterClient>());
        return services;
    }

    // ── ChangeHero ────────────────────────────────────────────────────────────────
    public static IServiceCollection AddChangeHero(this IServiceCollection services, IConfiguration config)
    {

        services.Configure<ChangeHeroOptions>(
        config.GetSection("ChangeHero"));

        services.AddHttpClient<IChangeHeroClient, ChangeHeroClient>(client =>
        {
            var seconds = config.GetValue<int>("ChangeHero:RequestTimeoutSeconds", 10);
            client.Timeout = TimeSpan.FromSeconds(Math.Clamp(seconds, 2, 30));
        });

        services.AddTransient<IExchangePriceApi>(sp => sp.GetRequiredService<IChangeHeroClient>());
        services.AddTransient<IExchangeBuyPriceApi>(sp => sp.GetRequiredService<IChangeHeroClient>());
        services.AddTransient<IExchangeCurrencyApi>(sp => sp.GetRequiredService<IChangeHeroClient>());
        return services;
    }
    ...
    public static IServiceCollection AddQuickEx(this IServiceCollection services, IConfiguration config)
    {
        ...
        services.Configure<QuickexOptions>(
            config.GetSection("Quickex"));
    ...
    public static IServiceCollection AddExolix(this IServiceCollection services, IConfiguration config)
    {
        ...
        services.AddHttpClient<IExolixClient, ExolixClient>((sp, client) =>
        {
            ...
            // Exolix: Authorization: {API Key} (no scheme). Passed on all requests for
            // affiliate attribution; it's harmless on the public /currencies and /rate.
